...
cd job-radar
./setup          # guided: asks name, email, level, locations
# drop your CV into profile/
claude .         # open in Claude Code → /job-scan
```

**Global install** — commands work from any project
```bash
cd job-radar && ./setup --global
```

**Add to an existing project**
```bash
cd job-radar && ./setup --install ~/Projects/my-project
```
