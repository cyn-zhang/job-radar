...
Level: {level} | Work type: {work_type} | Locations: {locations}

## Scan Summary
Sources checked | Total found | Eligible matches
...
Run Module 1, then output a formatted digest the user can copy to email:

**Subject:** `🎯 Job Digest {YYYY-MM-DD} | {X} roles | {Y} strong fits`

Include: Top Picks (with why + link), full results table, scanner notes, tip of the day.
